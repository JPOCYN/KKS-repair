...
test("loads Hostinger-first defaults with absolute storage paths", () => {
  ...
  assert.equal(config.appOrigin, undefined);
...
test("preserves future storage configuration and rejects unknown values", () => {
  const config = loadAppConfig({
    ...
    PUBLIC_ORIGIN: "https://supercardocs.com",
    APP_ORIGIN: "https://app.supercardocs.com",
  });
  ...
  assert.equal(config.publicOrigin, "https://supercardocs.com");
  assert.equal(config.appOrigin, "https://app.supercardocs.com");
  assert.equal(config.configuredOrigins, "https://supercardocs.com,https://app.supercardocs.com");
