...
const assetVersion = "20260829-auth-access-1";
...
export function adminCodesView(user: SessionUser, codes: Array<Record<string, unknown>>, saved = false, generated = 0): string {
  ...
  const rows = codes.map((row) => {
    ...
    return `<tr data-code-admin-row data-code-state="${state}" data-search="${escapeHtml(String(row.code || "").toLowerCase())}"><td><code>${escapeHtml(row.code)}</code></td><td>${escapeHtml(row.duration_hours)}</td><td><span class="status-pill status-pill--${state === "available" ? "active" : state === "used" ? "neutral" : "disabled"}">${label}</span></td><td><a href="/admin/codes/${escapeHtml(row.id)}/edit">Edit</a></td></tr>`;
  }).join("");
  ...
  const codeTable = `<div class="table-wrap"><table><thead><tr><th>Authorization code</th><th>Access hours</th><th>State</th><th></th></tr></thead><tbody>${rows}</tbody></table><div class="empty table-empty" data-code-empty hidden>No authorization codes match this filter.</div></div>`;
